...
const Auth = () => {
  ...
  const validateEmail = (value) => {
    ...
    if (!value.endsWith('@gokulp.online')) return 'Email must be a gokulp.online address'
    const re = /^[^\s@]+@gokulp\.online$/
    ...
    >
      ...
      >
        <motion.div className="flex items-center justify-between mb-6" layout>
          <h2 className="text-xl font-semibold text-white">{mode === 'login' ? 'Login' : 'Sign Up'}</h2>
          ...
          <div className="flex items-center justify-between">
            <AnimatePresence exitBeforeEnter>
              {mode === 'login' ? (
                ...
                >
                  <a href="#" className="text-yellow-300 hover:text-yellow-200 transition">Forgot Password?</a>
                </motion.div>
              ) : (
